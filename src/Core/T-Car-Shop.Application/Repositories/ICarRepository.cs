...
namespace T_Car_Shop.Application.Repositories
{
    ...
    {
        Task<CarEntity> GetByIdAsync(Guid id, CarSpecification specification, CancellationToken cancellationToken = default);
        Task<PagedData<CarEntity>> GetAllAsync(CarsSpecification specification, GetCarsFilterModel filter, CancellationToken cancellationToken = default);
